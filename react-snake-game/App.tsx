...
const App: React.FC = () => {
  ...
  const gameLoop = useCallback(() => {
    ...
    setSnake(prevSnake => {
      ...
      if (head.x === apple.x && head.y === apple.y) {
        ...
        setSpeed(prevSpeed => Math.max(MIN_SPEED, prevSpeed - speedIncrement));
  ...
  }, [apple, isGameOver, difficulty, createRandomApple, playSound]);
  ...
  }, [gameLoop, speed, difficulty, isGameOver]);
  ...
  useEffect(() => {
    ...
    if (snake.length > 0) {
      ...
      switch (directionRef.current) {
        case Direction.UP:
          ctx.fillRect(headX + eyeOffset1, headY + eyeOffset1, eyeSize, eyeSize);
          ctx.fillRect(headX + eyeOffset2, headY + eyeOffset1, eyeSize, eyeSize);
          break;
        case Direction.DOWN:
          ctx.fillRect(headX + eyeOffset1, headY + eyeOffset2, eyeSize, eyeSize);
          ctx.fillRect(headX + eyeOffset2, headY + eyeOffset2, eyeSize, eyeSize);
          break;
        case Direction.LEFT:
          ctx.fillRect(headX + eyeOffset1, headY + eyeOffset1, eyeSize, eyeSize);
          ctx.fillRect(headX + eyeOffset1, headY + eyeOffset2, eyeSize, eyeSize);
    ...
    ctx.beginPath();
    ctx.arc(apple.x * TILE_SIZE + TILE_SIZE / 2, apple.y * TILE_SIZE + TILE_SIZE / 2, TILE_SIZE / 2.2, 0, 2 * Math.PI);
    ctx.fill();
  }, [snake, apple]);
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-gray-900 font-mono p-4">
      ...
      <div className="relative bg-gray-800 border-4 border-green-500 shadow-lg shadow-green-500/20 rounded-lg">
        <div className="absolute -top-12 left-0 w-full flex justify-center items-center gap-4">
          ...
          <button onClick={() => setIsMuted(!isMuted)} className="bg-gray-900 border-2 border-green-500 px-3 py-1 rounded-md text-2xl" aria-label="Toggle Sound">
            {isMuted ? '🔇' : '🔊'}
          </button>
        </div>
        
        <canvas ref={canvasRef} width={CANVAS_WIDTH} height={CANVAS_HEIGHT} className="rounded-md" />
          ...
          <div className="absolute inset-0 bg-black bg-opacity-70 flex flex-col items-center justify-center rounded-lg p-4">
            {isGameOver ? (
              <div className="text-center">
                ...
                <button onClick={() => startGame(difficulty!)} className="mt-8 px-6 py-3 bg-green-500 text-white font-bold rounded-lg hover:bg-green-600 transition-colors text-xl">
                  Restart
                </button>
              </div>
            ) : (
              <div className="text-center">
                <h2 className="text-4xl font-bold text-green-400 mb-6">Select Difficulty</h2>
                <div className="flex flex-col gap-4">
                  <button onClick={() => startGame('Easy')} className="px-8 py-4 bg-blue-500 text-white font-bold rounded-lg hover:bg-blue-600 transition-colors text-2xl">
                    Easy
                  </button>
                  <button onClick={() => startGame('Medium')} className="px-8 py-4 bg-yellow-500 text-white font-bold rounded-lg hover:bg-yellow-600 transition-colors text-2xl">
                    Medium
                  </button>
                  <button onClick={() => startGame('Hard')} className="px-8 py-4 bg-red-500 text-white font-bold rounded-lg hover:bg-red-600 transition-colors text-2xl">
                    Hard
                  </button>
                </div>
              </div>
